...
This version of the task was updated by Matt Nassar on July 31 2026 to create a version of the cannon task with a 2 x 2 design manipulating temporal structure (changepoint/oddball) and initialization of prediction (previous prediction versus random push). The key file for running the new version of the task is runCNTRCSversion.m, and the files and dependencies (psych toolbox) are on matlab path, it can be run by entering the following command in the matlab command window: runCNTRCSversion(0, 1, 1);
